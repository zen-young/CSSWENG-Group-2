...
function Edit_Product({ setPages, productName }) {
  ...
  function removeImage(index) {
    ...
    if (index < arrLen1 && arrLen1 > 0) {
      setProdURLS((productURLS) =>
        productURLS.filter((img_url, i) => i !== index)
      );
      if (arrLen1 - 1 == 0) setProdURLS([]);
    } else if (index >= arrLen1 && arrLen1 > 0) {
      setNewUrls((newUrls) =>
        newUrls.filter((img_url, i) => i !== index - arrLen1)
      );
    ...
    return (
      <div key={key} className="flex w-full h-auto">
        ...
        <ul id="sizes" className="w-full align-middle">
          ...
          {feature == 1 ? sizeInputs : feature == 2 ? paperInputs : colorInputs}
        </ul>
      </div>
    );
  }
  ...
  function init_Features() {
    sizes.map((val, index) => {
      if (index != 0 && sizeInputs.length == 0)
        add_entry(sizeInputs, setSizeInputs, 1, val);
    });

    paperTypes.map((val, index) => {
      if (index != 0 && paperInputs.length == 0)
        add_entry(paperInputs, setPaperInputs, 2, val);
    });

    colors.map((val, index) => {
      if (index != 0 && colorInputs.length == 0)
        add_entry(colorInputs, setColorInputs, 3, val);
    });
  }
  ...
  return (
    <>
      ...
      <div className="relative flex flex-col w-full h-full bg-gray-100 pr-[40px] ">
        {/* Confirm Delete */}
        <div
          className={`absolute w-full h-full z-10 bg-black bg-opacity-80 ${
        ...
        >
          <div className="absolute top-1/3 left-1/4 w-1/2 h-auto my-auto mx-auto bg-white opacity-100 px-2 py-2 rounded-md">
            <div className="flex-col text-center">
              <img
                className="w-[50px] mx-auto mb-[20px]"
